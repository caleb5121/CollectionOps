...
export default function LandingFinalCTA({ animate, tone = "dark" }: Props) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="mt-9 flex w-full flex-col items-center sm:mt-10 lg:mt-11"
        initial={animate ? { opacity: 0, y: 14 } : false}
        whileInView={animate ? { opacity: 1, y: 0 } : undefined}
        viewport={{ once: true, margin: "-40px" }}
        transition={{ duration: 0.45, delay: 0.2, ease }}
      >
        <motion.div whileHover={animate ? { scale: 1.03 } : undefined} whileTap={animate ? { scale: 0.98 } : undefined} transition={{ type: "spring", stiffness: 400, damping: 22 }}>
          <Link
            href="/data?demo=1"
